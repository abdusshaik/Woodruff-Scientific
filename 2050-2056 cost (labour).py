...
# Calculate the accumulated costs over 6 years
accumulated_costs = np.cumsum(total_costs_per_year)

# Calculate the cost of one watt produced each year
power_output_per_year = 1e9  # 1 Giga watt
cost_per_watt_per_year = np.divide(accumulated_costs, power_output_per_year)

# Calculate the overall cost of one watt produced
overall_cost_per_watt = accumulated_costs[-1] / power_output_per_year
...
# Print the total costs of laborers and the cost of one watt produced for each year
for i, year in enumerate(years):
    print(f"Total costs of laborers in {year}: ${total_costs_per_year[i]:.2f}")
    print(f"Cost of one watt produced in {year}: ${cost_per_watt_per_year[i]:.10f}")

# Print the overall cost of one watt produced
print(f"Overall cost of one watt produced: ${overall_cost_per_watt:.10f}")
